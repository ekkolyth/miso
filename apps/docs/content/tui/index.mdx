...
When no terminal is attached — CI, a piped command, `docker` without `-t`, or an agent — miso prints `no interactive terminal — running plain` and runs the script with normal inherited stdio instead of the TUI. Nothing hangs or crashes.
