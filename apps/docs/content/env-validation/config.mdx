...
Both behaviors are miso-mode only. In a delegated repo (`repo: "turbo"` or `"nx"`) the delegate owns env, so `miso env` just type-checks the declared root entries — `scope` isn't required and nothing is injected.
