...
Path to this scope's override file, relative to the project root (or the member's directory for a member-local entry). Used by `miso env -g -o`: its values are layered on top of the populated baseline, winning on key conflict. One override file per scope — at the root level for `"global"`, or one per member entry.
