...
- **string** — character length. `min` defaults to `1`; `max` is unbounded unless you set it.
...
Miso's JSON schema flags unknown `VarConfig` keys in your editor; at runtime, unrecognized keys are ignored.
