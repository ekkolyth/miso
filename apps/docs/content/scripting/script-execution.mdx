...
> `.ts` runs on `bun` when a bun lockfile is present, otherwise `node`. Bare `node script.ts` needs Node 23.6+ (or 22.6+ with `--experimental-strip-types`); `bun` runs TypeScript with no extra setup.
