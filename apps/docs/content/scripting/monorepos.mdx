...
**2. Leave `repo` unset (or set it to `"miso"`).** Workspace discovery happens automatically — no monorepo-specific `repo` mode is required. Native fan-out and `@scope` runs execute through Miso's TUI — see [Running Workspace Scripts](#running-workspace-scripts). Set `repo` to `"turbo"` or `"nx"` only if you want to delegate orchestration to one of those tools instead.
...
|---|---|
| `name` | the scope declaring it — a root list resolves at root, a member list resolves at that member |
| `#name` | the repo root, explicitly, regardless of declaring scope |
| `@member/name` | the named workspace member, regardless of declaring scope |
...
- A name claimed by both `turbo.json` and a Miso script now errors instead of the script silently winning.
