...
The repo root uses miso in simple mode (`"packageManager": false`). Workspace membership is auto-detected from the root `package.json` `workspaces` field, so all workspace scripts are available via `miso <script> @workspace` without any `repo` setting.
