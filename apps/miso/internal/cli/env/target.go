...
func BuildTargetEnv(projectRoot string, cfg config.Config, target workspace.Target) ([]string, error) {
	...
	// Scoped env-value injection is a miso-mode feature; delegated (turbo/nx)
	// repos let the delegate own env. The node_modules/.bin PATH augmentation
	// below still runs in every mode so scripts can find local binaries.
	if !cfg.IsDelegated() {
		if len(cfg.Env) == 0 {
